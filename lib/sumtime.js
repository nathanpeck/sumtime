 ...
 * @constant
 */
Sumtime.prototype.resolutions = ['year', 'month', 'week', 'day', 'hour', 'minute', 'second'];
 ...
 * @method
 * @param {Moment} time - The Moment timestamp to fetch a datastore key for
 * @param {array} resolutionList - List of resolutions to fetch keys for
 */
Sumtime.prototype.resolutionKeys = function (time, resolutionList) {
  return resolutionList.map(function (resolution) {
    return [resolution, this.periodKey(time, resolution)];
  });
};
...
Sumtime.prototype.increment = function (key, time, value, resolution, callback) {
  ...
  var resolutionsToStoreAt;
  var index = this.resolutions.indexOf(resolution);
  if (index != -1) {
    resolutionsToStoreAt = this.resolutions.slice(0, index + 1);
  }
  else {
    resolutionsToStoreAt = this.resolutions;
  }

  async.each(
    // List of period counters to increment.
    self.resolutionKeys(parsedTime, resolutionsToStoreAt),
    // Increment a single time period's counter
    function (periodKey, done) {
...
Sumtime.prototype.getRange = function (key, startTimestamp, endTimestamp, resolution, callback) {
  ...
  for (var i = 0; i <= iterations; i++) {
    ...
    timeKeysToGet.push(this.periodKey(startMoment, resolution));
    startMoment.add(resolution, 1);
  }
